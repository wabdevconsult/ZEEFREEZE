...
const QuoteNewListPage = () => {
  const [searchTerm, setSearchTerm] = useState('');
  const [filterType, setFilterType] = useState('all');
  const { newRequests, confirmRequest, rejectRequest } = useQuotes();
  ...
  const handleConfirm = async (id: string) => {
    setConfirmingId(id);
    try {
      await confirmRequest.mutateAsync(id);
    } finally {
  ...
  const filteredRequests = newRequests.data?.filter(request => {
    const matchesSearch = 
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Devis - Nouvelles demandes</h1>
        <p className="text-gray-600">Gérez les nouvelles demandes de devis en attente</p>
      </div>
          ...
          <div className="bg-blue-50 rounded-lg p-4">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-xl font-semibold text-gray-900">{newRequests.data?.length || 0}</p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-xl font-semibold text-gray-900">
                  {new Set(newRequests.data?.map(r => r.companyId)).size || 0}
                </p>
              </div>
            </div>
          </div>
      ...
      {newRequests.isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredRequests.map((request) => (
            <QuoteRequestCard
              ...
              isConfirming={confirmingId === request.id}
        ...
        <div className="bg-white rounded-lg shadow-sm p-12 text-center">
          ...
          <p className="text-gray-500 mb-6">
            Il n'y a actuellement aucune nouvelle demande de devis à traiter.
          </p>
          <Link
            to="/dashboard/quotes/create"
            className="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700"
...
export default QuoteNewListPage;