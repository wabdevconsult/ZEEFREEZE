...
const QuoteCreatePage = () => {
  ...
  const { getRequest, createQuote, materialKits } = useQuotes();
  const { companies } = useCompanies();
  const { data: request, isLoading: isRequestLoading } = getRequest(requestId || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate('/dashboard/quotes/confirmed');
    } catch (error) {
